...
export async function pickReportFolder(): Promise<string | null> {
  const payload = await fetchJson<{ path: string | null }>(
    `${API_BASE_URL}/api/workspace/pick-report`,
    {
      method: "POST",
    }
  );
...
export async function runTests(
  ...
  const body: Record<string, unknown> = { testFiles, mode, parallelism };
  const trimmed = reportStoragePath?.trim();
  if (trimmed) {
    body.reportStoragePath = trimmed;
  }
